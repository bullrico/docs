...
description: "Stop forcing Rails to do AI. Learn how to build a FastAPI service for AI calls and integrate it properly with your Rails app, with real production code."
---
...
# Configure GPU layers based on environment
...
The best way to manage multiple services is with Docker Compose. Create a `compose.yml` (or `docker-compose.yml` for legacy compatibility):
...
Create a `Dockerfile` in your `rails_app` directory:
...
Create a `Dockerfile` in your `ai_service` directory:
...
RUN CMAKE_ARGS="-DLLAMA_CUDA=on" pip3 install llama-cpp-python==0.3.2 --force-reinstall --no-cache-dir
...
### Option 2: On-Premise
If you have your own GPU server, deploy using the same Docker setup. For Apple Silicon Macs in production, you'll need to modify the Dockerfile to use the standard Python image instead of CUDA:

```dockerfile
FROM python:3.11-slim  # For Apple Silicon
WORKDIR /app
COPY requirements.txt .
RUN pip install -r requirements.txt
# Install llama-cpp-python with Metal support
RUN CMAKE_ARGS="-DLLAMA_METAL=on" pip install llama-cpp-python==0.3.2 --force-reinstall --no-cache-dir
COPY . .
EXPOSE 8001
